...
public class TestMojo extends AbstractMojo {
    ...
    private static boolean isTruthy(String str) {
        return str == null?false:(str.equalsIgnoreCase("")?false:(str.equalsIgnoreCase("false")?false:(str.equalsIgnoreCase("no")?false:(str.equalsIgnoreCase("off")?false:!str.equalsIgnoreCase("disable")))));
    }
        ...
        if(isTruthy(skipTests)) {
            ...
            if(this.preconditions != null) {
                ...
                for(Iterator var10 = this.preconditions.entrySet().iterator(); var10.hasNext(); ++idx) {
                    ...
                    if(key.equals("and")) {
                        if(!isTruthy(val)) {
                            this.getLog().info("Skipping test " + this.testName + " because precondition number " + idx + " was not met.");
                            return false;
                        }
                    } else {
                        ...
                        if(isTruthy(val)) {
                            this.getLog().info("Skipping test " + this.testName + " because negative precondition number " + idx + " was met.");
                            return false;
                        }
                    }
                }
        ...
        if(this.shouldRunTest()) {
            if(!this.results.isDirectory() && !this.results.mkdirs()) {
                ...
                try {
                    ...
                    throw new MojoExecutionException("IOException while executing the test " + this.testName, var24);
                } catch (InterruptedException var25) {
                    throw new MojoExecutionException("Interrupted while executing the test " + this.testName, var25);
                } finally {
                    if(testThread != null) {
                ...
                this.getLog().info("STATUS: " + status + " after " + TimeUnit.MILLISECONDS.convert(end - start, TimeUnit.NANOSECONDS) + " millisecond(s).");
                this.getLog().info("-------------------------------------------------------");
