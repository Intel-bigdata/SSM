...
void load_erasurecode_lib(char* err, size_t err_len) {
  ...
  // Load Intel ISA-L
  #ifdef UNIX
  isaLoader->libec = dlopen(SMART_ISAL_LIBRARY, RTLD_LAZY | RTLD_GLOBAL);
  if (isaLoader->libec == NULL) {
    snprintf(err, err_len, "Failed to load %s (%s)",
                             SMART_ISAL_LIBRARY, dlerror());
    return;
  }
  ...
  #ifdef WINDOWS
  isaLoader->libec = LoadLibrary(SMART_ISAL_LIBRARY);
  if (isaLoader->libec == NULL) {
    snprintf(err, err_len, "Failed to load %s", SMART_ISAL_LIBRARY);
    return;
  }
  #endif
