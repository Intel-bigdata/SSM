...
#	    sh prepare_ec.sh
        su hdfs -c "sh prepare_ec.sh"
        cd ${PAT_HOME}/PAT-collecting-data          
        echo "start_time=\`date +%s\`;\
        su hdfs -c \"hadoop distcp -skipcrccheck -m ${MAPPER_NUM} ${SRC_CLUSTER}/${case}/* ${SRC_CLUSTER}${DEST_DIR_EC}/${case}/ > results/$case_${MAPPER_NUM}_$i.log 2>&1\";\
        end_time=\`date +%s\`;\
