...
import org.smartdata.server.engine.LegacyCopyScheduler;
...
@Deprecated
public class TestLegacyCopyScheduler extends MiniSmartClusterHarness {
  ...
  public void testForceSync() throws Exception {
    ...
    LegacyCopyScheduler legacyCopyScheduler = new LegacyCopyScheduler(ssm.getContext(),
    ...
    // init forceSync
    legacyCopyScheduler.forceSync(srcPath, destPath);
    legacyCopyScheduler.start();
    while (legacyCopyScheduler.getCachedSize() == 0) {
      Thread.sleep(100);
    }
    while (ssm.getCmdletManager().getActionsSizeInCache() != 0 ||
        legacyCopyScheduler.getCachedSize() != 0) {
      System.out.printf("Size of cmdlet %d\n", ssm.getCmdletManager().getActionsSizeInCache());
      System.out.printf("Size of copy %d\n", legacyCopyScheduler.getCachedSize());
  ...
  public void testSplitCopyFile() throws IOException {
    ...
    List<CopyTargetTask> copyTargetTaskList =
        LegacyCopyScheduler.splitCopyFile(srcPath + "/" + file1,
            destPath + "/" + file1, 1,
