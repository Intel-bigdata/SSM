...
angular.module('dashboard')
  ...
  .controller('WorkerCtrl', ['$scope', '$propertyTableBuilder', '$sortableTableBuilder',
    ...
    function ($scope, $ptb, $stb, i18n, helper, models, locator, worker0, apps0) {
      ...
      $scope.whatIsCmdlet = i18n.terminology.workerExecutor;
      $scope.executorsTable = {
        cols: [
          ...
          $stb.link('Executor').key('cmdlets').canSort().styleClass('col-xs-4').done(),
      ...
      function updateExecutorsTable() {
        $scope.executorsTable.rows = $stb.$update($scope.executorsTable.rows,
          _.map($scope.worker.cmdlets, function (executor) {
            if ($scope.rules.hasOwnProperty(executor.appId)) {
              ...
              return {
                ...
                cmdlets: {href: executorPageUrl, text: executorPath},
      ...
      worker0.$subscribe($scope, function (worker) {
        ...
        _.forEach(worker.cmdlets, function (executor) {
