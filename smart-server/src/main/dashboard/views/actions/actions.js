  ...
  .controller('ActionsCtrl', ['$scope', '$modal', '$state', '$sortableTableBuilder', '$dialogs', 'actions0', 'actionTypes',
    function ($scope, $modal, $state, $stb, $dialogs, actions0, actionTypes) {
      ...
      var submitWindow = $modal({
        templateUrl: 'views/actions/submit/submit.html',
      ...
      $scope.actionsTable = {
        cols: [
          ...
          // group 2/3 (5-col)
          $stb.datetime('Create Time').key('createTime').canSort().sortDefaultDescent().done(),
          $stb.datetime('Finish Time').key('finishTime').canSort().sortDefaultDescent().done(),
