    ...
    function ($scope, $modal, $state, $stb, $dialogs, rules0) {
      ...
      $scope.rulesTable = {
        cols: [
          ...
          // group 2/3 (5-col)
          $stb.datetime('Submission Time').key('submitTime').canSort().sortDefaultDescent().styleClass('col-md-1 hidden-sm hidden-xs').done(),
          $stb.datetime('Last Check Time').key('lastCheckTime').canSort().sortDefaultDescent().styleClass('col-md-1 hidden-sm hidden-xs').done(),
          $stb.text('Checked Number').key('numChecked').canSort().done(),
          $stb.text('Commands Generated').key('numCmdsGen').canSort().done(),
