...
public class TestRuleRestApi extends IntegrationTestBase {
  ...
  public void test() throws Exception {
    Response response1 = RestAssured.get(RULEROOT + "/list");
    String json1 = response1.asString();
    response1.then().body("status", equalTo("OK")).
        body("body.size", is(0));

    String rule = "file : every 1s | age > 2m | archive";
    Response res = RestAssured.post(RULEROOT + "/add?args=" + rule);
