...
public class SmartDFSInputStreamBak extends DFSInputStream {
  private FileContainerInfo containerFileInfo;

  public SmartDFSInputStreamBak(DFSClient dfsClient, String src,
...
//  }
   ...
   */
  void openInfo3() throws IOException, UnresolvedLinkException {
  ...
  private long fetchLocatedBlocksAndGetLastBlockLength() throws IOException {
    final LocatedBlocks newInfo = dfsClient.getLocatedBlocks(src,
        containerFileInfo.getOffset(), containerFileInfo.getLength());
  ...
  public int read(long position, byte[] buffer, int offset, int length)
    ...
    try {
      return pread(position + containerFileInfo.getOffset(), buffer, offset, length);
    } finally {
      scope.close();
    }
  }
