...
macro(smart_add_compiler_flags FLAGS)
    set(CMAKE_C_FLAGS "${CMAKE_C_FLAGS} ${FLAGS}")
    set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} ${FLAGS}")
endmacro()

# Add flags to all the CMake linker variables.
macro(smart_add_linker_flags FLAGS)
    set(CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} ${FLAGS}")
    set(CMAKE_SHARED_LINKER_FLAGS "${CMAKE_SHARED_LINKER_FLAGS} ${FLAGS}")
endmacro()

# Compile a library with both shared and static variants.
function(smart_add_dual_library LIBNAME)
...
function(smart_target_link_dual_libraries LIBNAME)
    target_link_libraries(${LIBNAME} ${ARGN})
    target_link_libraries(${LIBNAME}_static ${ARGN})
endfunction()

# Set all the output directories to the same place.
function(smart_output_directory TGT DIR)
...
function(smart_dual_output_directory TGT DIR)
    smart_output_directory(${TGT} "${DIR}")
    smart_output_directory(${TGT}_static "${DIR}")
endfunction()
...
macro(smart_set_find_shared_library_version LVERS)
...
# afterwards. On Windows this function is a no-op.  Windows does not encode
# version number information information into library path names.
macro(smart_set_find_shared_library_without_version)
    if(${CMAKE_SYSTEM_NAME} MATCHES "Darwin")
        # Mac OS uses .dylib
...
smart_add_compiler_flags("${GCC_SHARED_FLAGS}")
smart_add_linker_flags("${LINKER_SHARED_FLAGS}")
    ...
    if(CMAKE_SYSTEM_PROCESSOR MATCHES "^arm")
        ...
        if(READELF MATCHES "NOTFOUND")
            ...
            if(NOT JVM_ELF_ARCH MATCHES "Tag_ABI_VFP_args: VFP registers")
                ...
                smart_add_compiler_flags("-mfloat-abi=softfp")
            endif()
        endif()
    endif()
...
elseif(CMAKE_SYSTEM_NAME STREQUAL "SunOS")
    ...
    smart_add_compiler_flags("-m64 -D_POSIX_C_SOURCE=200112L -D__EXTENSIONS__ -D_POSIX_PTHREAD_SEMANTICS")
    set(CMAKE_C_FLAGS "-std=gnu99 ${CMAKE_C_FLAGS}")
    set(CMAKE_CXX_FLAGS "-std=gnu++98 ${CMAKE_CXX_FLAGS}")
    smart_add_linker_flags("-m64")
