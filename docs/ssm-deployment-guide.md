...
To distribute `${SMART_HOME}` directory to SSM Server nodes and each Smart Agent nodes, you should enter into ${SMART_HOME} directory and type `./bin/install.sh`. Smart Agent can coexist with Hadoop HDFS Datanode. For better performance, We recommend to deploy one agent on each Datanode. Of course, Smart Agents on servers other than Datanodes and different numbers of Smart Agents than Datanodes are also supported.
